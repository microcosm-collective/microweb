...
def single(request, event_id):
    ...
    if hasattr(event, "when"):
        ...
        is_expired = (
            True if int(end_date.strftime("%s")) < int(today.strftime("%s")) else False
        )
    else:
    ...
    view_data = {
        ...
        "pagination": build_pagination_links(
            responses[event_url]["comments"]["links"], event.comments
        ),
        "item_type": "event",
        "attendees": attendees,
...
def csv(request, event_id):
    ...
    """
    return redirect(
        build_url(request.get_host(), ["events", "%s", "attendeescsv"]) % (event_id,)
        + "?access_token="
...
def create(request, microcosm_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                event_response = event_request.create(
            ...
            if len(invites.strip()) > 0:
                ...
                if len(invited_list) > 0:
                    for userid in invited_list:
                        if userid != "":
                            attendees.append(
                                {"rsvp": "maybe", "profileId": int(userid)}
                            )
                    if len(attendees) > 0:
                        try:
...
def edit(request, event_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                event_response = event_request.update(
                    request.get_host(), request.access_token
                )
            except APIException as exc:
                return respond_with_error(request, exc)
    ...
    if request.method == "GET":
        try:
            event = Event.retrieve(
                request.get_host(), id=event_id, access_token=request.access_token
            )
        except APIException as exc:
            return respond_with_error(request, exc)
        ...
        try:
            view_data["attendees"] = Event.get_attendees(
                host=request.get_host(), id=event_id, access_token=request.access_token
            )
...
def delete(request, event_id):
    ...
    event = Event.retrieve(
        request.get_host(), event_id, access_token=request.access_token
    )
    try:
...
def geocode(request):
    ...
    if "q" in request.GET:
        response = GeoCode.retrieve(
            request.get_host(), request.GET["q"], request.access_token
        )

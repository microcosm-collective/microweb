        ...
        if not request.access_token:
            ...
            view_data = {
                ...
                "pagination": build_pagination_links(
                    responses[url]["updates"]["links"], updates_list.updates
                ),
                "site_section": "updates",
                "site": Site(responses[request.site_url]),
            }
...
class WatcherView:
    ...
    def list(request):
            ...
            if "watcher_id" in request.POST:
                ...
                for w in watchers:
                    if request.POST.get("delete_watcher_" + str(w)):
                        ...
                        Watcher.update(
                            request.get_host(), int(w), postdata, request.access_token
                        )
        ...
        if request.method == "GET":
            ...
            view_data = {
                ...
                "pagination": build_pagination_links(
                    responses[url]["watchers"]["links"], watchers_list.watchers
                ),
            }
    ...
    def single(request):
        postdata = {
            "updateTypeId": int(
                request.POST.get("updateTypeId")
            ),  # used on create for communication defaults
        ...
        if request.POST.get("delete"):
            ...
            response = Watcher.update(
                request.get_host(), postdata, request.access_token
            )
...
class UpdatePreferenceView:
    ...
    def settings(request):
        ...
        if request.method == "GET":
            ...
            request.view_requests.append(
                grequests.get(url2, params=params2, headers=headers2)
            )

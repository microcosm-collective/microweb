...
def single(request, huddle_id):
    ...
    request.view_requests.append(
        grequests.get(huddle_url, params=params, headers=headers)
    )
    try:
    ...
    view_data = {
        ...
        "pagination": build_pagination_links(
            responses[huddle_url]["comments"]["links"], huddle.comments
        ),
        "item_type": "huddle",
...
def list(request):
    ...
    view_data = {
        ...
        "unread": unread,
        "pagination": build_pagination_links(
            responses[huddle_url]["huddles"]["links"], huddles.huddles
        ),
    }
...
def create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                hud_response = hud_request.create(
            ...
            if request.POST.get("invite"):
                ...
                Huddle.invite(
                    request.get_host(), hud_response.id, ids, request.access_token
                )
            ...
            return HttpResponseRedirect(
                reverse("single-huddle", args=(hud_response.id,))
            )
        else:
            view_data["form"] = form
            return render(request, form_template, view_data)
    ...
    if request.method == "GET":
        if request.GET.get("to"):
            ...
            for recipient_id in list_of_recipient_ids:
                try:
                    recipient_profile = Profile.retrieve(
                        request.get_host(), recipient_id
                    )
                except APIException:
...
def delete(request, huddle_id):
    ...
    try:
        huddle = Huddle.retrieve(
            request.get_host(), huddle_id, access_token=request.access_token
        )
